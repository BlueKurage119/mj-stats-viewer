...
> - ツールチップトリガーの限定: ラベルの点線下線を撤去し、`.stats-row__info-btn`（Material Icon `info`）を配置。ホバー時のマウスカーソルは `help` とする。PC では `@media (hover: hover)` 内で hover / focus-visible で表示し、タッチ端末ではタップで表示・非表示をトグルする（`.stats-row[data-tip-open="true"]`。sticky hover 防止のため hover ルールを分離。2回目のタップで閉じる際に `blur()` を実行してフォーカスを解除）。外側タップ（`pointerdown`）で閉じる。
