...
> - 2カラムグリッド化（タブレット以上）: PC等の広い画面幅（`@media (min-width: 840px)`）における間伸び解消のため、`.stats-panel` を `display: grid; grid-template-columns: repeat(2, 1fr); align-items: start; gap: 16px;` に切り替え。対戦数カウントは `grid-column: 1 / -1;` で全幅配置。行数の多い `[data-section="riichi"]`（16行）および `[data-section="luck"]`（12行）は `grid-row: span 2;` で2行分の高さに割り当て、隙間なくバランスよく収める。840px未満は既存の1カラムを維持。
