...
| **R-13** | **和銃分布セクションで回数を出すか**。Issue #12 は「凡例に回数を出さない（% のみ）」と判断していた（`issue-12-win-lose-donuts.md` §0 R-1） | **2026-09-07 オーナー指示により、9行すべてで「回数 / 割合」を併記する。ただし回数が API に実在するのは和了時の3キーだけで、残り6行の回数は概算であるため「約」を付けて区別する**（§2.2-8） | #12 が回数を出さなかったのは**サマリーカードの凡例**という限られた面積で書式を揃えるためで、参照タブである本タブとは前提が違う。本タブは「可能な限り数値を見せる」ことが責務なので、概算であることを明示したうえで回数を出す。**CLI の `formatWinLossDistribution` を模倣してはならない**（§1.7）。**2026-09-07 追検証（§1.7 全面改訂）: 実データ4サンプルで `D = 放铳率 × roundCount` が「放銃時」3行の母数と厳密に一致することを確認した。一方「放銃相手」3行の母数は D より 0.8〜5.3% 大きい（ダブロン・トリロンで 1 discard が複数クレジットになるため）。概算方式は維持できるが、注記の文言を訂正した** |
...
各行の回数 = Math.round(D × その行の率)
```

- **放銃時3行**: `D` が母数そのものなので、丸め誤差だけの正確な値になる。
- **放銃相手3行**: 真の母数はクレジット数（= `D` の 1.008〜1.053 倍）なので、`D` を使うと**系統的に 1〜5% 少なく出る**。丸め後は行あたり最大で数回のずれになる（サンプル2の最大行で 90 vs 94）。**「約」表記と、ダブロンを明示した注記でこの過小を利用者に伝える**（§2.2-8）。クレジット数を推定する手段は API に無いため補正はしない（補正係数を捏造しない）。
- **和了時3行**: API の生カウント（`立直和了` ほか）をそのまま出す。合計 `W` が `和牌率 × roundCount` と一致することは実データで確認済みなので、注記に「和了回数 {W} 回のうちの実測値」と書いてよい。
...
| `dealInTargetRiichi` | `放铳至立直` | 放銃相手 立直 | **概算** `round(D × 放铳至立直)` | `放铳至立直`（API 生の率） | 確 |
| `dealInTargetCall` | `放铳至副露` | 放銃相手 副露 | **概算** `round(D × 放铳至副露)` | `放铳至副露`（API 生の率） | 確 |
| `dealInTargetDamaten` | `放铳至默听` | 放銃相手 闇聴 | **概算** `round(D × 放铳至默听)` | `放铳至默听`（API 生の率） | 確 |
...
- `D = round(放铳率 × roundCount)`（**放銃した discard イベント数**。`放铳率` の分母は配牌回数＝`extended.roundCount`）
  - **放銃時3行では `D` が母数そのもの**（実データ4サンプルで厳密一致・§1.7.3-1）。回数は丸め誤差だけの正確な値になる
  - **放銃相手3行の真の母数は「(discard, 和了者) のペア数＝クレジット数」で、`D` より 0.8〜5.3% 大きい**（ダブロン・トリロン。§1.7.3-2）。`D` を使うと**系統的に少なく出る**。API にクレジット数を得る手段が無いため補正はしない

**表示書式（実測値と概算を見た目で区別する・R-13 の必須要件）**:

| 種別 | `valueText` の例 | 規則 |
|---|---|---|
| 実測（和了時3行） | `21回 / 36.2%` | 回数に**「約」を付けない** |
| 概算（残り6行） | `約12回 / 26.5%` | 回数に**必ず「約」を付ける** |

**注記（ツールチップ）** — 本セクションは9行すべてに注記を入れる（R-14 で `note` は空許容になったが、このセクションは母数の説明が必須のため空にしない）:

| 行 | 注記 |
|---|---|
| 和了時3行 | `和了回数 {W} 回のうちの実測値` |
| 放銃時3行 | `放銃回数 {D} 回（放銃率 × 局数）× 各割合。API に回数の生データが無いため丸めた値` |
| 放銃相手3行 | `放銃回数 {D} 回（放銃率 × 局数）× 各割合。ダブロン・トリロンのぶん実際の和了者数は放銃回数より数%多いため、やや少なめに出る` |

- 注記の `{W}` / `{D}` は**動的に埋める**（`statsMetrics.ts` の静的な `note` ではなく `statsView.ts` が組み立てる）。静的テーブル側の `note` には `{W}` / `{D}` を含まない固定文を置き、ビューが数値入りに差し替える。**このセクションの9行は空文字にしない。**
- **放銃時3行と放銃相手3行で注記の文面を変える**（前者は丸めのみ、後者はダブロンによる過小を明記）。同一文言の使い回しは A9-9 で落ちる。
...
| `放铳率` が非有限 / `0` / `roundCount` が非有限 → `D` が算出できない | 概算6行を **割合だけ**表示（`— / 26.5%`）。回数部を捏造しない |
| 個々の率が非有限 | その行を `—`（行は消さない・R-5） |

**CLI を模倣しないこと（§1.7.5）**: CLI の `formatWinLossDistribution` は `放铳至*`（率）を合計して `totalDealIn` とし、`Math.round(totalDealIn × 放铳时立直率)` を「回数」として出しているが、`totalDealIn` は常に約 1.0 なのでこの回数は常に 0〜1 になる。**バグである。**本設計は `D = 放铳率 × roundCount` を使う。
...
- [ ] **A9-2** **概算と実測が見た目で区別されていること**（R-13 の必須要件）。実行: `#/__stats` で
  ...
  ```
  → 9件。うち `winStateRiichi` / `winStateCall` / `winStateDamaten` の3件は **「約」を含まない**、
  残り6件（`dealInState*` 3件 + `dealInTarget*` 3件）は**すべて「約」で始まる回数**を含むこと
...
- [ ] **A9-5** 単体テスト: `放铳率` が `0` / `undefined` / `NaN` のいずれかのとき、
  概算6行が `— / {割合}` になり **回数を捏造しないこと**。`roundCount` が欠落した場合も同じ
- [ ] **A9-6** **CLI のバグを模倣していないこと**（§1.7）。実行:
  `grep -nE "放铳至立直.*\+.*放铳至副露|totalDealIn" src/stats/` → **0件**
  （`放铳至*` 3値を合計して母数にしていないこと。あれは率の合計で常に約 1.0 になる）
- [ ] **A9-7** 単体テスト: 概算回数の母数が `放铳率 × roundCount` であること。
  `grep -n "放铳率" src/stats/statsView.ts` → 概算母数の算出箇所でヒットすること。
  かつ `放铳率` を 2 倍にしたテスト入力で概算6行の回数がすべて約2倍になること（母数依存の証明）
